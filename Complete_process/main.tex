...
\definecolor{coverbg}{HTML}{0F2744}
...
\begin{tabular}{cc}
\textcolor{accentteal}{\faShieldAlt} \, ElectionGuard Powered &
...
\begin{itemize}
  ...
  \item Whether the user \uifield{can create elections} (a permission that can be granted independently of role)
\end{itemize}
...
\begin{enumerate}
  ...
  \item Toggle \uifield{Can Create Elections} if applicable.
...
\begin{enumerate}
  \item Prepare a \texttt{.csv} file with the following columns:\\
    \texttt{email, role, can\_create\_elections}\\
    Example row: \texttt{alice@example.com, admin, true}
  ...
  \item Review the preview table that appears --- verify counts and roles.
  \item Click \uibutton{Confirm Import}.
\end{enumerate}

\begin{tcolorbox}[warnbox]
Duplicate email entries in the CSV will be ignored. If an email already exists in the system, its role will be \textbf{updated} to the value in the CSV, not duplicated.
\end{tcolorbox}
...
\begin{enumerate}
  ...
  \item Click your profile avatar in the top-right corner and select \uimenu{Profile Settings}.
  \item Scroll to the \uimenu{Security} section and click \uibutton{Enable Two-Factor Authentication}.
  \item A QR code will be displayed on screen.
  \item Open the Google Authenticator app on your smartphone.
  ...
  \item Click \uibutton{Confirm} to activate 2FA.
\end{enumerate}

\begin{tcolorbox}[warnbox]
\textbf{Back up your 2FA recovery codes!} AmarVote will display one-time recovery codes after 2FA is enabled. Print or save them somewhere safe. If you lose access to your authenticator app and have no recovery codes, you may be permanently locked out of your account.
\end{tcolorbox}
...
\begin{enumerate}
  ...
  \item Check your email for a password reset link (valid for 15 minutes).
  \item Click the link and set a new password.
\end{enumerate}
...
\begin{enumerate}
  ...
  \item Enter the \uifield{Party Name} (optional).
...
\begin{enumerate}
  \item Log in to AmarVote.
  \item You will see a notification indicating you have been assigned as a guardian. Navigate to \uimenu{All Elections} and find the election.
  \item Open the election page and click the \uimenu{Guardian} tab.
  ...
  \item Your browser will automatically \textbf{download an encrypted private key file} (\texttt{.json} or \texttt{.enc}) to your computer.
...
\begin{tcolorbox}[dangerbox]
\textbf{Critical --- Do not lose the following:}
\begin{enumerate}
  \item The downloaded private key file
  \item Your guardian password
\end{enumerate}
If you lose \emph{either}, you will not be able to participate in decryption. Depending on the quorum, this could prevent the election results from ever being tallied.
\end{tcolorbox}

\begin{tcolorbox}[tipbox]
Store the private key file in a secure location --- an encrypted USB drive, a secure cloud vault, or your system's encrypted home directory. Store your guardian password in a password manager.
\end{tcolorbox}
...
\begin{enumerate}
  ...
  \item Enter your \uifield{Guardian Password} in the provided field. The system will temporarily decrypt your private key in memory to generate the shares.
  ...
  \item The system will:
    \begin{enumerate}
      ...
      \item Submit all encrypted fragments to the server.
    \end{enumerate}
  \item A confirmation message will appear when Phase 2 is complete for you.
\end{enumerate}

\begin{tcolorbox}[warnbox]
Your password and decrypted private key are used only in-browser during this step. They are \textbf{never transmitted} to the server in decrypted form. The server only receives the encrypted backup shares.
\end{tcolorbox}
...
\begin{enumerate}
  \item Log in as the election administrator.
  \item Navigate to the election's management page.
...
AmarVote uses an \textbf{end-to-end verifiable} voting protocol. Your ballot is encrypted \emph{before} it is submitted to the server --- the server never sees how you voted in plain text.

\section{Accessing the Voting Booth}

\begin{enumerate}
  \item Log in to AmarVote during the election period.
  \item Navigate to \uimenu{All Elections} and find the election you wish to vote in.
...
\begin{enumerate}
  ...
  \item A confirmation dialog will appear. Review and click \uibutton{Confirm \& Cast Vote}.
\end{enumerate}
...
\begin{enumerate}
  ...
  \item In the \uimenu{Tally} or \uimenu{Results} tab, click \uibutton{Initiate Tally}.
...
\begin{enumerate}
  ...
  \item Enter your \uifield{Guardian Password}.
  \item Click \uibutton{Submit Credentials \& Decrypt}.
  \item The system will:
    \begin{enumerate}
      \item Decrypt your private key in-browser using your password.
      ...
      \item Submit all partial and compensated decryptions to the server.
    \end{enumerate}
  \item A confirmation message is displayed when your decryption contribution is submitted.
\end{enumerate}

\begin{tcolorbox}[warnbox]
The partial decryption process may take several minutes depending on the number of tally chunks. Do not close the browser tab while decryption is running.
\end{tcolorbox}
...
\begin{enumerate}
  \item Navigate to the election's \uimenu{Tally} tab.
...
\begin{enumerate}
  \item Navigate to the election page and open the \uimenu{Verification} tab.
  \item Click \uimenu{Verify My Ballot}.
  \item Enter your \uifield{Ballot Tracking Code} (from the receipt email).
  \item Enter your \uifield{Ballot Hash} (from the receipt email).
  \item Click \uibutton{Verify}.
\end{enumerate}
...
\begin{enumerate}
  ...
  \item The download package includes:
    \begin{itemize}
      \item All encrypted ballots
...
\begin{tcolorbox}[notebox]
Any technically capable third party can use this downloaded data to re-verify the entire election independently using the ElectionGuard specification. This is the gold standard of election transparency.
\end{tcolorbox}
...
Your voter receipt email contains a Ballot Tracking Code and Hash. These identifiers are mathematically derived from the \emph{encrypted} ballot --- they do not reveal your identity or your vote. The public tally list contains tracking codes but no voter names.
...
Transit security & HTTPS/TLS (Nginx production configuration) \\
