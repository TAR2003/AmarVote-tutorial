...
\begin{center}
	\begin{tabularx}{0.95\textwidth}{>{\bfseries}l X}
		...
		App Admin                               & A platform-level administrator who can create and manage elections on AmarVote. There can be an unlimited number of App Admins on the platform.                                                                                                                                                                                    \\
		\rowcolor{lightgray}Election Admin      & The specific user who created a particular election; responsible for configuring, scheduling, and overseeing that election. Each election has exactly one Election Admin. Note: ``Election Admin'' is simply a term used to identify the creator of a specific election --- the user's platform role remains App Admin (or Owner). \\
		Guardian                                & Participates in the cryptographic key ceremony; holds a share of the decryption key needed to tally results.                                                                                                                                                                                                                       \\
		\rowcolor{lightgray}Voter               & Registers, receives election access, casts an encrypted ballot, and independently verifies the result.                                                                                                                                                                                                                             \\
		\bottomrule
	\end{tabularx}
\end{center}
...
\vspace{0.5cm}
\begin{tcolorbox}[notebox]
	\textbf{Permission hierarchy:} Owner $>$ App Admin $>$ User (voter). The Owner can elevate or restrict any account. All roles require a registered account. An \textbf{Election Admin} is the creator of a specific election --- there is exactly one Election Admin per election --- but this does not change their platform-level role (App Admin or Owner).
\end{tcolorbox}

% ════════════════════════════════════════════════════════════════════════════
\mainmatter

% ─── WORKFLOW OVERVIEW (shown before all chapters) ───────────────────────────
\chapter*{AmarVote Workflow Overview}
\addcontentsline{toc}{chapter}{AmarVote Workflow Overview}
...
\screenfig{amarvote_workflow_1.png}{AmarVote end-to-end workflow --- Part 1: platform setup, election creation, and guardian key ceremony}

\screenfig{amarvote_workflow_2.png}{AmarVote end-to-end workflow --- Part 2: voting, tallying, decryption, and verification}
...
\begin{itemize}
	...
	\item The \uifield{assigned role} (Owner, App Admin, or User)
\end{itemize}
...
\begin{enumerate}
	...
	\item Enter the email address in the \uifield{Add User Email} field.
	\item Select the appropriate role from the \uifield{Role} dropdown.
	\item Click \uibutton{Add}.
\end{enumerate}
...
\begin{enumerate}
	...
	\item Navigate to \uimenu{Authenticated Users} $\to$ \uibutton{Upload CSV}.
	\item Click and select your prepared CSV file.
\end{enumerate}
...
\begin{center}
	\begin{tabularx}{0.95\textwidth}{>{\bfseries}l XXXX}
		\toprule
		\rowcolor{primaryblue}\color{white}Capability           & \color{white}Owner                  & \color{white}Admin (App)            & \color{white}User                   \\
		\midrule
		\rowcolor{lightgray}Make anyone owner                   & \textcolor{successborder}{\faCheck} & \textcolor{dangerborder}{\faTimes}  & \textcolor{dangerborder}{\faTimes}  \\
		Demote any owner                                        & \textcolor{successborder}{\faCheck} & \textcolor{dangerborder}{\faTimes}  & \textcolor{dangerborder}{\faTimes}  \\
		\rowcolor{lightgray}Manage authorised-user list         & \textcolor{successborder}{\faCheck} & \textcolor{successborder}{\faCheck} & \textcolor{dangerborder}{\faTimes}  \\
		Assign roles                                            & \textcolor{successborder}{\faCheck} & \textcolor{successborder}{\faCheck} & \textcolor{dangerborder}{\faTimes}  \\
...
\begin{tcolorbox}[notebox]
	\textbf{Election creation setting:} The Owner can enable a global setting allowing \emph{all} users (not just App Admins) to create elections. When this setting is active, both App Admins and regular Users will see the \uimenu{Create Election} button in their menu.
\end{tcolorbox}
...
\begin{center}
	\textbf{Home} \;\faArrowRight\; \textbf{Register} \;\faArrowRight\; \textbf{Enter Email} \;\faArrowRight\; \textbf{Code Sent} \;\faArrowRight\; \textbf{Enter Code} \;\faArrowRight\; \textbf{Set Password} \;\faArrowRight\; \textbf{Account Created}
\end{center}
...
\subsection{Step 1 --- The AmarVote Home Page}

\begin{enumerate}
	\item Open your web browser and navigate to the AmarVote application URL provided by your organisation.
	\item You will arrive at the \textbf{AmarVote home page}.

	      \screenfig{homeAmarVote.png}{The AmarVote home page --- your entry point to the platform}

	\item Click the \uibutton{Register} button, or navigate directly to \texttt{/register} in your browser's address bar.
\end{enumerate}

\subsection{Step 2 --- Submit Your Email Address}

\begin{enumerate}
	\item The registration form will open, prompting you for your email address.

	      \screenfig{verifyEmailForRegistering.png}{The registration page --- enter the email address that was added to the Authorised User List}

	\item Enter your \uifield{Email Address} in the provided field. This must \textbf{exactly match} the email address the Owner or Admins added to the Authorised User List.
	\item Click \uibutton{Send Verification Code}.
	\item The system will dispatch a \textbf{One-Time Password (OTP)} to your inbox and display a confirmation screen.

	      \screenfig{codeSentToEmail.png}{The verification code has been sent to your email}
\end{enumerate}

\begin{tcolorbox}[tipbox]
	Check your spam or junk folder if you do not receive the email within two minutes. The OTP is typically valid for \textbf{10 minutes}. If it expires, return to the registration page and request a new code.
\end{tcolorbox}

\subsection{Step 3 --- Enter the Verification Code}

\begin{enumerate}
	\item Open the email from AmarVote and locate the \textbf{6-digit verification code}.
	\item Return to the registration page (it may still be open in your browser).
	\item Enter the code in the \uifield{Verification Code} field.

	      \screenfig{enterTheVerificationCode.png}{Entering the 6-digit OTP received by email to verify your address}

	\item Click \uibutton{Verify Code}.
	\item If the code is correct and has not expired, you will be automatically advanced to the password setup step.
\end{enumerate}
...
\begin{enumerate}
	\item The password creation screen will appear.

	      \screenfig{setYourPassword.png}{The password setup screen --- choose a strong password for your AmarVote account}

	\item Enter a strong password in the \uifield{Password} field. A strong password contains at least \textbf{8 characters} and mixes uppercase letters, lowercase letters, numbers, and symbols.
	\item Re-enter the identical password in the \uifield{Confirm Password} field.
	\item Click \uibutton{Create Account}.
	\item Your account will be created and you will be redirected to the login page (or automatically signed in, depending on your platform configuration).
\end{enumerate}

\begin{tcolorbox}[dangerbox]
	\textbf{Never share your password.} AmarVote staff will never ask for your password. Store it securely in a trusted password manager such as Bitwarden, 1Password, or your browser's built-in vault.
\end{tcolorbox}

\section{Two-Factor Authentication (2FA) --- Optional but Strongly Recommended}

After your account is created, you can enable Two-Factor Authentication (2FA) using \textbf{Google Authenticator} or any TOTP-compatible app (such as Authy or Microsoft Authenticator). 2FA adds a critical second layer of security --- even if someone learns your password, they cannot log in without your physical device.

\subsection{Enabling 2FA}

\begin{enumerate}
	\item Log in to your new account.
	\item Click your profile in the top-right corner.

	      \screenfig{goToProfileMenu.png}{Accessing the profile menu from the top-right corner}

	\item Click \uibutton{Turn on} Two-Step Verification. The 2FA setup screen will appear.

	      \screenfig{enable2FactorAuthentication.png}{The Two-Factor Authentication setup screen, displaying the QR code to scan with your authenticator app}
	...
	\item Point your camera at the QR code displayed on screen.
	\item Your authenticator app will register AmarVote and begin displaying a rotating \textbf{6-digit code}.
	\item Enter the current code in the \uifield{Verification Code} field on AmarVote and confirm.
	\item Successful input will activate 2FA on your account immediately.
\end{enumerate}

\begin{tcolorbox}[warnbox]
	\textbf{Safeguard your authenticator access.} If you lose access to the device running your authenticator app, you may be permanently locked out of your AmarVote account. Consider storing a backup code in a secure location, or using an authenticator app that supports encrypted cloud backup.
\end{tcolorbox}
...
\section{Login Workflow Overview}

\begin{center}
	\textbf{Home} \;\faArrowRight\; \textbf{Login} \;\faArrowRight\; \textbf{Enter Credentials} \;\faArrowRight\; \textbf{Dashboard}
\end{center}

If Two-Factor Authentication is enabled, an additional step is inserted after credential entry:

\begin{center}
	\textbf{Home} \;\faArrowRight\; \textbf{Login} \;\faArrowRight\; \textbf{Enter Credentials} \;\faArrowRight\; \textbf{Enter 2FA Code} \;\faArrowRight\; \textbf{Dashboard}
\end{center}

\section{Standard Login}

\begin{enumerate}
	\item Open your web browser and navigate to the AmarVote application URL. You will arrive at the \textbf{AmarVote home page}.

	      \screenfig{homeAmarVote.png}{The AmarVote home page --- click \textit{Login} to proceed to the sign-in screen}

	\item Click \uibutton{Login} on the home page, or navigate directly to \texttt{/login}.
	\item The \textbf{login screen} will appear.

	      \screenfig{loginScreen.png}{The AmarVote login screen --- enter your registered email address and password}

	\item Enter your registered \uifield{Email Address}.
	\item Enter your \uifield{Password}.
	\item Click \uibutton{Continue}.
	\item Upon successful authentication, you will be redirected to your \textbf{Dashboard}.
\end{enumerate}
...
\begin{enumerate}
	\item Complete Steps 1--5 of the Standard Login above (navigate to home, go to login screen, enter email and password).
	\item Click \uibutton{Continue}.
	\item A prompt will appear asking for your \uifield{Authenticator Code}.
	\item Open the Google Authenticator (or equivalent TOTP) app on your smartphone and copy the current \textbf{6-digit code} displayed for AmarVote.
	\item Enter the code in the prompt and click \uibutton{Verify}.
	\item Upon successful verification, you will be redirected to your Dashboard.
\end{enumerate}

\begin{tcolorbox}[notebox]
	TOTP codes rotate every \textbf{30 seconds}. If the code expires while you are typing, simply wait for the next code to appear in your authenticator app and use that one instead.
\end{tcolorbox}
...
\begin{enumerate}
	...
	\item Check your inbox for an email containing a \textbf{password reset verification code} (valid for a limited time).
	\item Enter the verification code on the reset page, then choose and confirm your new password.
	\item Once set, you may log in immediately with your new credentials.
\end{enumerate}
...
This chapter is intended for App Admins (and Users with election-creation permission). It walks through every option available when setting up a new election. The user who creates an election automatically becomes its \textbf{Election Admin} --- the sole administrator responsible for managing that specific election.

\begin{tcolorbox}[warnbox]
	The \uimenu{Create Election} button will only appear in your navigation menu if you have been granted election-creation permission. If you cannot see it, contact AmarVote Owner or Admin.
\end{tcolorbox}
...
\screenfig{createElectionTitleDescription.png}{Election creation form --- title, description, and maximum choice configuration}
...
\begin{enumerate}
	\item Prepare a CSV file with one column: \texttt{just email addresses}.\\
	...
	\item Click \uibutton{Upload CSV} and select your file.
...
\begin{enumerate}
	\item Prepare a CSV file with one column: \texttt{just email addresses} (one guardian email per row).

	      \screenfignarrow{guardiansCSVFile.png}{Example guardians CSV file format --- one guardian email per row}

	\item Click \uibutton{Choose File} and select your file.
...
\begin{enumerate}
	...
	\item Recommended: set quorum to at least $\lfloor n/2 \rfloor + 1$ where $n$ is the total guardian count.
\end{enumerate}
...
\begin{enumerate}
	...
	\item Upload a \uifield{Candidate Photo} (optional, JPG/PNG, max 10 MB).
...
\begin{enumerate}
	\item Review all the information given in the page.
...
\begin{enumerate}
	...
	\item Click \uibutton{Generate Credentials} as many times as you like.
	\item Every time you click, the system will cryptographically generate a new \textbf{asymmetric key pair} (a public key and a private key) specifically for this election.
	...
	\item \textbf{Important:} Click \uibutton{Generate Password} to let the system generate a random, strong password for you, \emph{or} enter your own password in the field.
	\item Click \uibutton{Submit Round 1}.
	\item Your browser will automatically \textbf{download an encrypted private key file} (\texttt{.txt}) to your computer. \textbf{You must allow downloading that file, else the election can not go forward}.
...
\begin{enumerate}
	...
	\item Click \uibutton{Choose File} and select the \texttt{.txt} file you downloaded in Phase 1.
	...
	\item The system will:
	      ...
	      \end{enumerate}
	\item Click \uibutton{Submit Round 2} to upload the generated shares to the database.
...
\section{Phase 3 --- Election Scheduling \& Notifications (Election Admin)}
\label{sec:phase3}
...
\begin{tcolorbox}[notebox]
	\textbf{Election Admin reminder:} The term \emph{Election Admin} refers specifically to the user who created this election. There is exactly one Election Admin per election. All other platform-level App Admins do not have administrative access to elections they did not create.
\end{tcolorbox}

\subsection{Election Admin Actions --- Phase 3}

\begin{enumerate}
	\item Log in with the Election Admin account (the account that created this election).
	\item Navigate to the election's \uimenu{Guardian} tab.
	\item You will see the \textbf{Phase 3: Activation By Admin} panel is now active.
\end{enumerate}

\screenfig{createElectionSetTimeAndActivateElection.png}{Phase 3 panel --- election scheduling and optional voter notification configuration}

\subsubsection{Step A --- Setting the Election Start and End Times}

The scheduling interface features a \textbf{date-and-time picker} designed for ease and precision:

\begin{enumerate}
	\item Click the \uifield{Election Start Date \& Time} field. A calendar widget will appear, allowing you to:
	      \begin{itemize}
		      \item Navigate months using the \textbf{left} and \textbf{right arrow} buttons.
		      \item Click any \textbf{date} to select it. The selected date highlights immediately.
		      \item Use the \textbf{hour} and \textbf{minute} scroll wheels (or type directly) to set the exact time. AM/PM toggle is available for 12-hour format, or the field accepts 24-hour format automatically.
	      \end{itemize}
	\item Repeat the same process for the \uifield{Election End Date \& Time} field.
	\item The system will validate that the end time is after the start time. An error message will appear immediately if the selection is invalid.
\end{enumerate}

\begin{tcolorbox}[warnbox]
	\textbf{Choose election times carefully.} Once the election is activated, the start and end times cannot be changed. Ensure you have accounted for your voters' time zones and any logistical constraints before proceeding.
\end{tcolorbox}

\subsubsection{Step B --- Configuring a Voter Reminder Email (Optional)}

Below the scheduling fields, you will find the \textbf{Send Reminder Email} section:

\begin{enumerate}
	\item \textbf{Enable the Reminder Email:} Check the \uifield{Send Reminder Email to Voters} checkbox. When checked, the full reminder configuration panel expands beneath it.

	\item \textbf{Email Receivers:}
	      \begin{itemize}
		      \item By default, \textbf{all voters on the election's voter list} are pre-populated as recipients. A scrollable list displays each recipient's email address, as there may be hundreds or thousands of voters.
		      \item Each recipient entry has a \textbf{\texttimes{} (remove) button} on its right side. Click it to remove that specific voter from the reminder list.
		      \item At the bottom of the receiver list, an \textbf{Add Receiver} field with an \uibutton{Add} button allows you to manually add any additional email address (e.g., a supervisor or observer not on the voter list).
	      \end{itemize}

	\item \textbf{Email Subject:}\\
	      The \uifield{Subject} field is \textbf{pre-filled with a professional subject line}, for example:\\
	      \emph{``Reminder: [Election Title] --- Voting is Now Open''}\\
	      You may edit this to suit your context.

	\item \textbf{Email Body:}\\
	      The \uifield{Message Body} text area is \textbf{pre-filled with a complete, professional email body} that automatically includes:
	      \begin{itemize}
		      \item The \textbf{election title}.
		      \item The \textbf{election description}.
		      \item The \textbf{direct link} to the election page on AmarVote.
		      \item The \textbf{voting start and end times}.
		      \item A closing note encouraging voters to reach out if they have questions.
	      \end{itemize}
	...
	\item \textbf{Reminder Send Time:}\\
	      Use the \uifield{Reminder Time} date-and-time picker to select \emph{when} the reminder emails will be sent. This uses the same modern, interactive picker as the start/end time fields:
	      \begin{itemize}
		      \item Select the date using the calendar widget.
		      \item Set the hour and minute using the scroll wheels or direct input.
		      \item The reminder time must be \textbf{before the election end time}.
		      \item The reminder time can be set \emph{before} the election start time (e.g., to notify voters in advance) or \emph{during} the election period.
	      \end{itemize}
\end{enumerate}

\screenfig{createElectionReminder.png}{Voter reminder email configuration --- receivers list with scroll and remove/add controls, pre-filled subject and body, and reminder send time picker}
...
\begin{tcolorbox}[notebox]
	If you do \emph{not} wish to send a reminder, simply leave the \uifield{Send Reminder Email to Voters} checkbox unchecked. The scheduling step will still be completed normally when you activate the election.
\end{tcolorbox}

\subsubsection{Step C --- Activating the Election}

\begin{enumerate}
	\item After setting the start and end times (and optionally configuring the reminder email), click \uibutton{Activate Election}.
	\item The election status changes to \textbf{Scheduled}. Voters will be able to vote starting from the specified start time.
	\item If a reminder was configured, the system will automatically dispatch the reminder emails at the specified reminder time.
\end{enumerate}

\screenfig{electionPageWithStartandFinishingTime.png}{Election page confirming the scheduled start and end times after activation}
...
% ────────────────────────────────────────────────────────────────────────────
\section{Editing the Voter List}
% \label{ch:edit-voters}

After an election has been created and scheduled, the \textbf{Election Admin} (the user who created the election) and any \textbf{assigned guardians} may still modify the voter list --- but only \emph{until the election start time}. Once voting begins, the voter list is locked and can no longer be changed.

\begin{tcolorbox}[notebox]
	\textbf{Who can edit the voter list?} The Election Admin and guardians assigned to this election. Both roles have the same voter-list editing privileges, and both are subject to the same deadline: all changes must be made before the scheduled election start time.
\end{tcolorbox}

\subsection{Accessing the Voter List Editor}

\begin{enumerate}
	\item Log in to AmarVote with an Election Admin or guardian account.
	\item Navigate to \uimenu{All Elections} and open the election you wish to manage.
	\item On the election page, locate the \uibutton{Edit Voter List} button.

	      \screenfig{electionPageFindEditVoters.png}{The election management page --- click \textit{Edit Voter List} to open the voter list editor}
\end{enumerate}

\subsection{Adding and Removing Voters}

The voter list editor allows you to add new voters or remove existing ones at any time before the election begins.

\screenfig{editVoters.png}{The voter list editor --- add voters manually or import a CSV file; remove voters using the cross button next to each email}

\subsubsection{Removing a Voter}

To remove a voter from the list:

\begin{enumerate}
	\item In the voter list editor, find the voter's email address in the list.
	\item Click the \textbf{cross (\texttimes{}) button} on the right side of the field containing that voter's email.
	\item The voter will be removed from the list immediately.
\end{enumerate}

\subsubsection{Adding Voters Manually}

To add a single voter by email:

\begin{enumerate}
	\item In the voter list editor, locate the \uifield{Add Voter Email Manually} field.
	\item Enter the voter's email address.
	\item Click \uibutton{Add Voter} to add them to the list.
\end{enumerate}

\subsubsection{Adding Voters via CSV Upload}

To add multiple voters at once:

\begin{enumerate}
	\item Prepare a CSV or plain-text file with one email address per row (same format as during election creation).
	\item In the voter list editor, click \uibutton{Import CSV/TXT}.
	\item Select your prepared file from your computer.
	\item The system will import all valid email addresses and add them to the voter list.
\end{enumerate}

\begin{tcolorbox}[warnbox]
	\textbf{Deadline:} Voter list editing is only available before the election start time. Once the election opens for voting, the voter list is permanently locked --- no further additions or removals are possible.
\end{tcolorbox}
...
AmarVote uses an \textbf{end-to-end verifiable} voting protocol. Your ballot is encrypted \emph{before} it is stored in the database in encrypted form, the database never stores your vote in plaintext.
...
\begin{enumerate}
	...
	\item Open the election page by clicking on the Election title you want to vote in.
...
\begin{enumerate}
	\item The voting booth displays all candidates with their names and photos (if uploaded).
...
\begin{enumerate}
	...
	\item A mismatch is flagged — this occurs if you entered wrong candidates or if the encryption is faulty (which should never happen in a properly functioning system).
\end{enumerate}
...
\begin{enumerate}
	\item After creating an encrypted ballot, click \uibutton{Challenge Vote} (Benaloh Challenge).
...
\begin{enumerate}
	...
	\item Click \uibutton{Cast Vote} (do \emph{not} click Challenge this time).
\end{enumerate}
...
\begin{center}
	\begin{tikzpicture}[every node/.style={font=\small,draw,rounded corners,text width=3.5cm,align=center}]
		\node[fill=infoblue] (T1) at (0,0) {Initiate Tally\\(Election Admin or Guardian)};
...
\begin{enumerate}
	...
	\item In the upper tab, click \uibutton{Create/Check Tally Status}.
	\item If the tally has not yet been created, you will see a \uibutton{Create Tally} button. Click it to start the tallying process.
	...
	\item The system will begin processing. This involves:
	      \begin{itemize}
		      ...
		      \item Store each of the tally chunks back to the database for later decryption.
	      \end{itemize}
...
\begin{enumerate}
	...
	\item Click \uibutton{Choose File} and select your private key file from Phase 1.
	...
	\item Click \uibutton{Submit Guardian Credentials}.
...
\section{Combining Decryption Shares (Election Admin or Guardian)}
...
\begin{enumerate}
	...
	\item The \uibutton{Combine Partial Decryptions} button will become active.
	\item Click \uibutton{Combine Partial Decryptions}.
...
\begin{enumerate}
	...
	\item Click \uibutton{Choose File} and upload your ballot receipt you received in the email after your encrypted ballot was cast.
...
\begin{enumerate}
	...
	\item You can download:
	      \begin{itemize}
		      ...
		      \item The encrypted tally of different chunks
		      \item All partial decryptions and proofs of different chunks
	      \end{itemize}
\end{enumerate}
...
Your voter receipt email contains a Ballot Tracking Code and Hash. Ballot hash is mathematically derived from the \emph{encrypted} ballot --- they do not reveal your identity or your vote. The public tally list contains tracking codes but no voter names.
...
\begin{center}
	\begin{tabularx}{0.95\textwidth}{>{\bfseries}l X}
		...
		\rowcolor{lightgray}Authentication           & JWT tokens                                                             \\
